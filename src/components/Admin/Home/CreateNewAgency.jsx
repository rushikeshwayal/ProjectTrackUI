...
function CreateNewAgency() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://127.0.0.1:5000/api/post/agency', formData, {
        headers: {
          ...
          <form onSubmit={handleSubmit} className="space-y-10 ">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <textarea
                  name="agency_professionals"
                  value={formData.agency_professionals}
                  onChange={handleChange}
                  className="w-full p-3 mt-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-purple-600"
                  required
                ></textarea>
